...
const fileFilter = (req, file, cb) => {
  ...
  if (allowedExts.includes(path.extname(file.originalname).toLowerCase())) cb(null, true);
  else cb(new Error("Only image files are allowed"));
};

const upload = multer({ storage, fileFilter, limits: { fileSize: 15 * 1024 * 1024 } }); // 15MB per file
