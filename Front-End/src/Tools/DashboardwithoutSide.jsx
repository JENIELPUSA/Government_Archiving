...
import { Package, DollarSign, Users, CreditCard, TrendingUp, FileText, UploadCloud, UserCheck, HardDrive, Sun, Moon, Clock, User, Info, Globe, ChevronLeft, ChevronRight } from 'lucide-react'; // Import additional lucide-react icons
import StatisticsCard from "../Components/AdminDashboard/dashboardcard"
import RecentDocumentsSection from "../Components/AdminDashboard/dashboardrecentDocs"
import LogsAndAuditSection from '../Components/AdminDashboard/dashboardLogsAudit';
...
// Component for the Log Detail Modal
const LogDetailModal = ({ log, onClose }) => {
  if (!log) return null; // Don't render if no log is selected
...
// Header Component
const DashboardHeader = ({ theme, toggleTheme, handleFeatureClick }) => (
  <header className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 mb-6 flex justify-between items-center border-b border-blue-200 dark:border-gray-700 transition-colors duration-300">
    ...
    <div className="flex items-center space-x-4">
      ...
      <button
        ...
        className="p-2 rounded-full bg-blue-500 text-white hover:bg-blue-600 transition-colors duration-300 hidden sm:block"
...
const UploadDocumentModal = ({
  ...
  const handleFileSelection = (files) => {
    ...
    if (newFiles.length > 0) {
      // For simulation, use the content of the first file if available
...
// Search Archive Modal Component
...
function App() {
  ...
  const initialDocuments = [
    ...
    },
    {
      ...
      fileType: 'XLS', // Added fileType for filtering
    ...
    },
    {
      ...
      fileType: 'PPT', // Added fileType for filtering
    ...
    {
      typeIcon: <span className="text-blue-500 font-bold text-xl">DOC</span>,
      fileName: 'Meeting Minutes 2025-07-01.docx',
      fileType: 'DOC',
      location: 'C:\\Archive\\Meetings...',
    ...
    {
      typeIcon: <span className="text-green-500 font-bold text-xl">XLS</span>,
      fileName: 'Inventory Report Q1 2025.xlsx',
      fileType: 'XLS',
      location: 'C:\\Archive\\Inventory...',
      lastModified: '2025-04-15',
      size: '2.2 MB',
      tags: ['inventory', 'report', 'q1', '2025'],
    },
    {
      typeIcon: <span className="text-red-500 font-bold text-xl">PDF</span>,
      fileName: 'Contract Agreement ABC Corp.pdf',
      fileType: 'PDF',
      location: 'C:\\Archive\\Legal...',
    ...
    {
      typeIcon: <span className="text-orange-500 font-bold text-xl">PPT</span>,
      fileName: 'Training Module v1.pptx',
      fileType: 'PPT',
      location: 'C:\\Archive\\Training...',
      lastModified: '2025-02-10',
      size: '1.1 MB',
      tags: ['training', 'module', 'v1'],
    },
    {
      typeIcon: <span className="text-blue-500 font-bold text-xl">DOC</span>,
      fileName: 'Department Memo 001-2025.docx',
      fileType: 'DOC',
      location: 'C:\\Archive\\Memos...',
      lastModified: '2025-01-05',
      size: '400 KB',
      tags: ['memo', 'department', '2025'],
    },
  ];
  ...
  const logsData = [
    ...
    {
      timestamp: '2025-07-04 10:30:00 AM',
      event: 'Document "Project Plan Alpha.pptx" updated by "ProjectLead"',
      user: 'ProjectLead',
      type: 'Update',
      ipAddress: '192.168.1.101',
      oldData: { title: 'Project Plan Alpha', status: 'Draft', version: '1.0' },
      newData: { title: 'Project Plan Alpha', status: 'Under Review', version: '1.1' }
    },
    {
      timestamp: '2025-07-03 04:00:00 PM',
      event: 'User "Admin" created new user "MarketingUser"',
      user: 'Admin',
      type: 'Create',
      ipAddress: '192.168.1.100',
      oldData: null,
      newData: { username: 'MarketingUser', role: 'Viewer', department: 'Marketing' }
    },
    {
      timestamp: '2025-07-02 11:20:00 AM',
      event: 'Document "Legal Brief 2024.pdf" deleted by "LegalDept"',
      user: 'LegalDept',
      type: 'Delete',
      ipAddress: '172.16.0.30',
      oldData: { fileName: 'Legal Brief 2024.pdf', caseId: 'CASE-2024-001' },
      newData: null
    },
    {
      timestamp: '2025-07-01 09:00:00 AM',
      event: 'Document "HR Policy Manual.docx" updated by "HRManager"',
      user: 'HRManager',
      type: 'Update',
      ipAddress: '10.0.0.60',
      oldData: { fileName: 'HR Policy Manual.docx', section: 'Leave Policy', lastReview: '2023-12-31' },
      newData: { fileName: 'HR Policy Manual.docx', section: 'Leave Policy', lastReview: '2025-06-30', changes: 'Updated sick leave policy' }
    },
  ];
  ...
  // Pagination states for Recent Documents
  const [currentPageDocuments, setCurrentPageDocuments] = useState(1);
  const [documentsPerPage] = useState(5); // 5 documents per page

  // Pagination states for Logs and Audit
  const [currentPageLogs, setCurrentPageLogs] = useState(1);
  const [logsPerPage] = useState(5); // 5 logs per page
  ...
  // Combine search and type filters for Recent Documents
  ...
  // Pagination logic for Recent Documents
  const indexOfLastDocument = currentPageDocuments * documentsPerPage;
  const indexOfFirstDocument = indexOfLastDocument - documentsPerPage;
  const currentDocuments = getFilteredAndTypedDocuments().slice(indexOfFirstDocument, indexOfLastDocument);
  const totalPagesDocuments = Math.ceil(getFilteredAndTypedDocuments().length / documentsPerPage);

  const paginateDocuments = (pageNumber) => setCurrentPageDocuments(pageNumber);

  // Pagination logic for Logs and Audit
  const indexOfLastLog = currentPageLogs * logsPerPage;
  const indexOfFirstLog = indexOfLastLog - logsPerPage;
  const currentLogs = logsData.slice(indexOfFirstLog, indexOfLastLog);
  const totalPagesLogs = Math.ceil(logsData.length / logsPerPage);

  const paginateLogs = (pageNumber) => setCurrentPageLogs(pageNumber);


  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 font-inter p-4 sm:p-6 lg:p-8 flex flex-col transition-colors duration-300">
      {/* Main Content Area */}
      <main className="flex-1">
        ...
        <RecentDocumentsSection
          ...
          totalPagesDocuments={totalPagesDocuments}
          currentPageDocuments={currentPageDocuments}
          paginateDocuments={paginateDocuments}
          selectedType={selectedType}
          uniqueDocumentTypes={uniqueDocumentTypes}
          setShowTypeFilterDropdown={setShowTypeFilterDropdown}
          showTypeFilterDropdown={showTypeFilterDropdown}
          handleTypeFilter={handleTypeFilter}
        />

        <LogsAndAuditSection
          currentLogs={currentLogs}
          totalPagesLogs={totalPagesLogs}
          currentPageLogs={currentPageLogs}
          paginateLogs={paginateLogs}
          setSelectedLogForDetail={setSelectedLogForDetail}
        />
      ...
      <UploadDocumentModal
        ...
        simulateAutoTagging={simulateAutoTagging}
        suggestedTags={suggestedTags}
        manualTagInput={manualTagInput}
        setManualTagInput={setManualTagInput}
