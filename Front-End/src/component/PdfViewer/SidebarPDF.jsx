...
const Sidebar = ({
  ...
  const previewLimit = 20; // number of pages per batch
    ...
    <nav className="print-hidden left-0 top-0 mb-8 flex max-h-[100vh] w-64 flex-col gap-4 overflow-y-auto rounded-[2rem] border-r bg-white/60 p-4 text-black shadow-[inset_0_2px_4px_rgba(255,255,255,0.2)] backdrop-blur-md dark:border-blue-700 dark:bg-slate-800/50 dark:text-white dark:backdrop-blur-md">
      ...
      <div className="flex flex-col gap-3">
        <div className="grid grid-cols-2 gap-2">
          <button onClick={onZoomIn} className="flex items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700" title="Zoom In">
            <ZoomIn className="h-5 w-5" />
          </button>
          <button onClick={onZoomOut} className="flex items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-blue-700" title="Zoom Out">
      ...
      <div className="grid w-full grid-cols-2 gap-2">
        <button onClick={onDownload} className="flex items-center justify-center rounded-lg bg-yellow-600 px-4 py-2 text-white hover:bg-yellow-700" title="Download">
          <Download className="h-5 w-5" />
        </button>

        <button onClick={onPrint} className="flex items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-white hover:bg-yellow-700" title="Print">
      ...
      <div className="mt-4 flex max-h-[50vh] flex-col items-center gap-2 overflow-y-auto">
        ...
        {fileUrl && numPages > 0 && (
          <Document file={fileUrl} onLoadError={(err) => console.error("PDF preview load error:", err)}>
            {pagesToRender.map((pageIndex) => (
              ...
              >
                ...
                />
                <p className="mt-1 text-center text-xs text-black dark:text-white">Page {pageIndex}</p>
              </div>
            ))}
          </Document>
        )}
