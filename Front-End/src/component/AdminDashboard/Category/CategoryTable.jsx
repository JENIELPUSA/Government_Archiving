...
const CategoryTable = () => {
    ...
    const handleConfirmDelete = async () => {
        ...
        setVerification(false);
        try {
            await deleteCategory(isDeleteID, "Deleted");
        } catch (error) {
    ...
    return (
        <div className="relative w-full space-y-6">
            ...
            <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                <table className="w-full">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {isLoading ? (
                            Array.from({ length: skeletonCount }).map((_, i) => <CategoryRowSkeleton key={i} />)
                        ) : isPageChanging ? (
                            Array.from({ length: ITEMS_PER_PAGE }).map((_, i) => <CategoryRowSkeleton key={i} />)
            ...
            {/* Inilipat ang StatusVerification modal sa labas ng isAddCategoryModal block */}
            <StatusVerification
