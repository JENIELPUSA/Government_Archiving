...
function DashboardLayout() {
  ...
  const { isFile, isTotaDocuments, isTodayDocuments, isMonthlyFile } = useContext(FilesDisplayContext);
  ...
  // ✅ Compute total storage from isMonthlyFile (safe)
  const getTotalStorageMB = () => {
    if (!Array.isArray(isMonthlyFile)) return 0;
    const totalBytes = isMonthlyFile.reduce(
      (sum, month) => sum + (typeof month.totalFileSize === "number" ? month.totalFileSize : 0),
      0
    );
  ...
  const statisticsData = [
    ...
    },
    {
      ...
      value: <AnimatedValue targetValue={getTotalStorageMB()} suffix=" MB" precision={2} />,
      ...
      <main className="mx-auto flex w-full flex-1 flex-col space-y-10 rounded-3xl bg-white/60 p-6 text-gray-900 shadow-inner backdrop-blur-md transition-colors duration-300 dark:bg-slate-800/50 dark:text-gray-100 dark:backdrop-blur-md">
        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          <div className="min-h-[300px] w-full overflow-hidden lg:w-1/2">
            <LineGraph isMonthlyFile={isMonthlyFile} />
          </div>
          <div className="min-h-[300px] w-full overflow-hidden lg:w-1/2">
