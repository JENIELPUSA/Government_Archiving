...
import { FaUser, FaCalendarAlt, FaTag, FaExternalLinkAlt, FaTimes, FaUndo } from "react-icons/fa";
...
export const DocumentDetailsPanel = ({ document, onClose }) => {
    ...
    const handdlerestore = async (item) => {
        setIsLoading(true);
        try {
            if (item.ArchivedStatus === "For Restore") {
                const result = await MOveArchived(item._id, "Active");
                if (result.success) {
                    onClose();
                }
            } else if (item.ArchivedStatus === "Archived") {
                const result = await MOveArchived(item._id, "Active");
                if (result.success) {
                    onClose();
                }
            } else {
                const result = await MOveArchived(item._id, "For Restore");
                if (result.success) {
                    onClose();
                }
            }
    ...
    const handleViewFile = (fileId, item) => {
        ...
        setIsLoading(true);
        // Delay navigation slightly to show loading overlay
        setTimeout(() => {
            navigate(`/dashboard/pdf-viewer/${fileId}`, { state: { fileData: item } });
            setIsLoading(false); // Hide loading overlay after navigation
    ...
    return (
        <div className="fixed inset-0 z-[999] flex items-center justify-center overflow-y-auto bg-black bg-opacity-50 p-4 dark:bg-gray-900/80">
            <div className="relative my-8 w-full max-w-3xl rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
                <button
                    onClick={onClose}
                ...
                <h2 className="mb-6 text-center text-3xl font-bold text-gray-800 dark:text-white md:text-left">Document Details</h2>
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <div>
                        <h3 className="mb-3 break-words text-2xl font-semibold text-blue-700 dark:text-blue-300">{document.title || "N/A"}</h3>
                        <p className="mb-5 leading-relaxed text-gray-600 dark:text-gray-400">{document.summary || "No summary available."}</p>
                        ...
                        <div className="mb-6 flex items-center text-base text-gray-700 dark:text-gray-300">
                            ...
                            <strong>Created At:</strong> {document.createdAt ? new Date(document.createdAt).toLocaleDateString() : "N/A"}
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-700/50">
                                ...
                                <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">{document.category || "N/A"}</p>
                            </div>
                            <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-700/50">
                                <p className="text-sm text-gray-500 dark:text-gray-400">Department</p>
                                <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">{document.department || "N/A"}</p>
                            </div>
                            <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-700/50">
                                <p className="text-sm text-gray-500 dark:text-gray-400">Status</p>
                                <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">{document.status || "N/A"}</p>
                            </div>
                            <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-700/50">
                                <p className="text-sm text-gray-500 dark:text-gray-400">Archived Status</p>
                                <p className="text-lg font-semibold text-gray-800 dark:text-gray-200">{document.ArchivedStatus || "N/A"}</p>
                            </div>
                        </div>
                        {document.tags && document.tags.length > 0 && (
                            <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-700/50">
                                <h4 className="mb-3 text-base font-medium text-gray-700 dark:text-gray-300">Tags</h4>
