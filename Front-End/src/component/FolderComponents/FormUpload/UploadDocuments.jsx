...
import { FiUploadCloud, FiFileText, FiBook, FiTag, FiCalendar, FiX, FiCheckSquare, FiPlus, FiHash, FiChevronDown, FiStar } from "react-icons/fi";
...
const UploadDocumentModal = ({ isOpen, onClose, folderId, isSuccess }) => {
    ...
    const handleUpload = async (e) => {
        ...
        if (includeAuthor && authors.length === 0 && !customAuthor) {
            setAuthorError("Please select or add at least one author.");
            valid = false;
        }
    ...
    const handleFinalSubmit = async () => {
        ...
        // ADDED: Append isFeatured to formData
        ...
        if (includeAuthor && authors.length > 0) {
            formData.append("author", JSON.stringify(authors));
        } else if (includeAuthor && customAuthor) {
            const customAuthorObj = {
                id: "custom",
                full_name: customAuthor,
                custom: true,
            };
            formData.append("author", JSON.stringify([customAuthorObj]));
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                setCustomViceChairperson("");
                setCustomAuthor("");
                setIsFeatured(false); // ADDED: Reset isFeatured
    ...
    const handleAddNewPerson = async () => {
        ...
        switch (currentModalType) {
            ...
            case "author":
                if (result && result._id) {
                    setAuthors((prev) => [...prev, result._id]);
                } else {
                    setCustomAuthor(displayName);
                }
                setAuthorError("");
                break;
            default:
    ...
    // Functions for vice chairpersons
    ...
    const handleAuthorToggle = (memberId) => {
        if (authors.includes(memberId)) {
            setAuthors(authors.filter((id) => id !== memberId));
        } else {
            setAuthors([...authors, memberId]);
        }
    };

    const handleRemoveAuthor = (memberId) => {
        setAuthors(authors.filter((id) => id !== memberId));
    };
            ...
            <div className="flex flex-col">
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        onClick={() => {
                                            ...
                                            } else if (label.includes("Vice")) {
                                                handleRemoveViceChairperson(memberId);
                                            } else if (label.includes("Author")) {
                                                handleRemoveAuthor(memberId);
                                            } else {
                ...
                >
                    <button
                        ...
                        className={`flex w-full items-center justify-between rounded-lg border px-4 py-2.5 text-left text-sm transition-all focus:outline-none focus:ring-2 ${error
    ...
    return (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/50 p-2 backdrop-blur-sm sm:p-4">
                            ...
                            <div className="space-y-4">
                                    ...
                                    {isFormLoading ? (
                                        ...
                                        <div className="relative">
                                            <textarea
                                                ...
                                                className={`min-h-[120px] w-full resize-none rounded-lg border px-4 py-2.5 text-sm transition-all focus:outline-none focus:ring-2 ${titleError
                                                    ? "border-red-400 focus:ring-red-300 dark:border-red-500"
                                                    : "border-gray-300 focus:border-blue-500 focus:ring-blue-200 dark:border-gray-600"
                                                    } dark:bg-gray-700/50 dark:text-gray-200`}
                                            ...
                                            {isFormLoading ? (
                                                ...
                                                <div className="relative">
                                                    <input
                                                        ...
                                                        className={`w-full rounded-lg border px-4 py-2.5 text-sm transition-all focus:outline-none focus:ring-2 ${resolutionNumberError
                                                            ? "border-red-400 focus:ring-red-300 dark:border-red-500"
                                                            : "border-gray-300 focus:border-blue-500 focus:ring-blue-200 dark:border-gray-600"
                                                            } dark:bg-gray-700/50 dark:text-gray-200`}
                                ...
                                <div className="rounded-lg border border-yellow-200 bg-yellow-50 p-4 dark:border-yellow-800 dark:bg-yellow-900/20">
                                    <label className="flex cursor-pointer items-center gap-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <FiStar className={`text-lg ${isFeatured ? 'text-yellow-600 fill-yellow-600' : 'text-yellow-500'}`} />
                                    ...
                                    {isFeatured && (
                                        <p className="mt-2 text-xs text-yellow-700 dark:text-yellow-300">
                                            <FiStar className="mr-1 inline-block text-xs" />
                                ...
                                {includeAuthor && (
                                    <CommitteeMemberDropdown
                                        ...
                                        onClearAll={handleClearAllAuthors}
                                        error={authorError}
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    {isFormLoading ? (
                                        ...
                                        <div
                                            className={`flex flex-col items-center justify-center rounded-lg border-2 border-dashed p-6 transition-all ${isDragging
            ...
            {(showChairpersonModal || showViceChairpersonModal || showAuthorModal) && (
                <AuthorModal
                    showAuthorModal={showChairpersonModal || showViceChairpersonModal || showAuthorModal}
                    ...
                    modalTitle={
                        currentModalType === "chairperson"
                            ? "Add New Chairperson"
                            : currentModalType === "viceChairperson"
                                ? "Add New Vice Chairperson"
                                : "Add New Author"
                    }
                    buttonText={
                        currentModalType === "chairperson"
